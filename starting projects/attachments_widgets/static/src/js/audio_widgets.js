odoo.define('audio_widget', function (require) {
    ...
    const FieldCharFile = require("web.basic_fields").FieldChar;
    ...
    const AttachmentSourceAudioWidget = FieldCharFile.extend({
        _renderReadonly: function () {
            this._super.apply(this, arguments);
            if (this.mode == "readonly")
                if (this.value) {
    ...
    fieldRegistry.add('attachment_source_audio_widget', AttachmentSourceAudioWidget);

    return {
        AttachmentMany2OneAudioWidget: AttachmentMany2OneAudioWidget,
        AttachmentBinaryAudioWidget: AttachmentBinaryAudioWidget,
        AttachmentSourceAudioWidget: AttachmentSourceAudioWidget
    };
})
